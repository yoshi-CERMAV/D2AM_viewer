...
public:
    ...
    void init_map()
    {
        ...
        if(Detector->masked){
            ...
            for(int i = 0; i!= Detector->image_size; i++) pixel_index[valid_count]=i;
                   ...
                   int *bg_in, int *dat, double scale, plot_data *plt, int &size)
    {
        ...
        for(int k = 0; k < kmax; k++){
            ...
            int i1 = i0;
            while (center_qb[pixel_index[i1]*2] < q1 && i1 < Detector->image_size) i1++;
            //            cout << i1 <<" "<<i0<<endl;
            size_t len = i1-i0;
    ...
    double fit(double q0, double q1){
        return fit(q0, q1, Detector->clean_data, Detector->clean_bg, Detector->response);
    }
...
protected:
    ...
    inline void init_integrate(double qmin, double qmax, double qstep_in, const char filename[])
    {
        qstep = qstep_in;
        fo.open(filename);
        do_write=true;
